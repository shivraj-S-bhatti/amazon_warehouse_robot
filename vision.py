...
class VisionNode(Node):
    ...
    def __init__(self):
        ...
        # solvePnP can give noisy lateral/distance estimates on the first
        # few frames. These constants gate and smooth raw detections before
        # they are exposed via get_marker(), preventing the robot from
        # reacting to a single bad estimate and spinning wildly.

        # Minimum consecutive frames a marker must be seen before we report it
        ...
        self._raw_detections = {}     # current-frame raw detections (internal)
        self._smoothed = {}           # per-marker EMA state {marker_id: detection_dict}
        self._frame_counts = {}       # consecutive-frame counter {marker_id: int}
    ...
    def _image_callback(self, msg):
        ...
        # ── Smooth and confirm detections before exposing via get_marker() ──
        #
        # Strategy:
        #   - Track how many consecutive frames each marker has been visible.
        #   - Apply EMA smoothing to lateral and distance so one noisy frame
        #     doesn't cause a sudden course correction.
        #   - Only promote a detection to self.detections (the public dict)
        #     once it has been seen for at least DETECTION_MIN_FRAMES frames.
        #   - Reset a marker's counter the moment it disappears from frame.
        ...
        for mid, det in new_detections.items():
            if mid in self._smoothed:
                # Blend new raw values into the running estimate
                prev = self._smoothed[mid]
                det['lateral']  = alpha * det['lateral']  + (1.0 - alpha) * prev['lateral']
                det['distance'] = alpha * det['distance'] + (1.0 - alpha) * prev['distance']
                det['vertical'] = alpha * det['vertical'] + (1.0 - alpha) * prev['vertical']
                self._frame_counts[mid] = self._frame_counts.get(mid, 0) + 1
            else:
                # First time we see this marker — start the counter
        ...
        # Drop smoothed state for markers that left the frame this tick
        disappeared = set(self._frame_counts.keys()) - set(new_detections.keys())
    ...
    # ── Smoothing / confirmation helpers ──────────────────────────────────

    def get_raw_marker(self, marker_id):
        """
        Get the unfiltered, unsmoothed detection for a marker from the
        current frame. Bypasses the EMA smoother and min-frame gate.

        Useful for debugging or for code that needs the instant raw pose.
        Normal callers should use get_marker() instead.
    ...
    def get_confirmation_count(self, marker_id):
        """
