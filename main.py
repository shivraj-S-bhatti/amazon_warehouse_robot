    ...
    def handle_search_object(self):
        ...
        if detection is not None:
            self.logger.info(
                ...
                f'{detection["distance"]:.2f}m — stopping to verify...')
            self.chassis.stop()

            # Brief settle: wait for the chassis to fully stop and for the
            # vision smoother to accumulate a few more stable frames before
            # we start driving toward the marker.  This prevents the robot
            # from acting on a noisy first-frame lateral estimate and spinning.
            time.sleep(0.5)

            # Re-read after the pause — if the marker is still visible and
            # the smoothed estimate looks sane, proceed; otherwise keep searching.
            detection = self.vision.get_marker(OBJECT_MARKER_ID)
            if detection is None:
                self.logger.info('Marker not confirmed after pause — continuing search')
                return STATE_SEARCH_OBJECT

            self.logger.info(
                f'Marker confirmed: dist={detection["distance"]:.2f}m  '
